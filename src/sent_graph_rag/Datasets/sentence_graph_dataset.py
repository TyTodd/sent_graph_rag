...
class SentenceGraphDataset:
    def __init__(self, nlp = None, verbose = False):
        if nlp is None:
            print("Initializing spacy model...")
            try:
                disable_progress_bar()
                self.nlp = spacy.load("en_core_web_sm")
                self.nlp.add_pipe("fastcoref",  config={'device': 'cuda:0', "enable_progress_bar": False})
                print("Spacy model loaded successfully.")
            except:
                raise ValueError("Could not load spacy model. Please install en_core_web_sm")
        else:
            self.nlp = nlp
    ...
    @staticmethod
    def from_dataset(datatset_reader: DatasetReader,  out_path: str, chunk_size: int = 1000, metadata: dict = {}, nlp = None, verbose = False):
        sentence_graph_dataset = SentenceGraphDataset(nlp, verbose)
        temp_path = os.path.splitext(out_path)[0] + "_temp" + os.path.splitext(out_path)[1]
        with open(temp_path, "wb") as out:
            writer(out, dataset_schema, sentence_graph_dataset.convert_dataset(datatset_reader, chunk_size, metadata = metadata))
        return sentence_graph_dataset
            
                
    def embed_dataset(self, in_path: str, out_path: str, chunk_size: int = 1000, metadata: dict = {}):
    ...
    def convert_dataset(self, dataset_reader: DatasetReader, chunk_size: int = 1000, metadata: dict = {}):
        texts = []
        all_qas = []
        processed_graphs = 0
        data_length = len(dataset_reader)
        with tqdm(total=data_length, desc="Converting dataset") as pbar:
            for row in dataset_reader.read():
                texts.append(row['context'])
                all_qas.append(row['qas'])
                processed_graphs += 1
                if len(texts) >= chunk_size or (processed_graphs >= data_length - 1 and len(texts) > 0):
                    graphs = self.create_graphs(texts)
                    while graphs:
                        yield {
